...
from .models import Plant, Comment,Requirement
...
def create_plants_view(request : HttpRequest):
	...
	if request.method == "POST":
		...
		new_plant.save()
		new_plant.requirements.set(request.POST.getlist('requirements'))

		return redirect('plants:all_plants_view')
	return render(request,'plants/create_plant.html',{"requirements": requirements})
...
def plant_update_view(request: HttpRequest, plant_id:int):
	...
	if request.method == "POST":
		...
		plants.requirements.set(request.POST.getlist('requirements'))
		if 'image' in request.FILES:
			plants.image = request.FILES['image']
		plants.save()
		return redirect('plants:plant_detail_view', plant_id)
	return render(request, "plants/plant_update.html", {"plants": plants, "requirements": requirements})
